...
function DemoCard({
    ...
    return (
        <motion.div
            ...
            onClick={onPlay}
            className={`ewiai-glass-card bg-[#0a0b1e] border overflow-hidden flex flex-col cursor-pointer transition-all duration-300 ${isPlaying ? 'border-blue-500/50 shadow-lg shadow-blue-500/10' : 'border-white/10 hover:border-blue-500/30'}`}
        >
            {/* Header */}
            <div className="p-4 border-b border-white/5 flex items-center justify-between">
                ...
                <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all shadow-lg ${isPlaying ? 'bg-blue-500' : 'bg-white'}`}>
                    {isPlaying ? (
            ...
            <div ref={containerRef} className={`p-4 flex-1 space-y-3 min-h-[200px] max-h-[300px] relative ${isPlaying ? 'overflow-y-auto' : 'overflow-hidden'}`}>
                {demo.conversation.map((line, idx) => (
                    ...
                    >
                        <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 text-[10px] font-bold ${line.role === 'ai' ? 'bg-blue-500 text-white' : 'bg-white/10 text-white'}`}>
                            {line.role === 'ai' ? 'AI' : 'C'}
                        </div>
                        <div className={`py-2 px-3 rounded-xl text-xs leading-relaxed max-w-[85%] ${line.role === 'ai' ? 'bg-white/5 text-white/80 rounded-tl-none' : 'bg-blue-500/10 text-blue-200 rounded-tr-none'} ${currentLineIndex === idx ? 'ring-1 ring-blue-500/40' : ''}`}>
                            {line.text}
                        </div>
                    </motion.div>
                ))}
...
export function EWIAIVoiceDemo() {
    ...
    return (
        <div className="w-full max-w-6xl mx-auto px-6">
            <div className="text-center mb-12">
                <h2 className="text-3xl md:text-5xl font-bold text-white mb-6 uppercase tracking-tight">AI Call Recordings.</h2>
                <p className="text-white/60 text-lg">Indistinguishable from a human agent. Listen to real conversations.</p>
            </div>
